...
def make_roi_alff_df(sim_alff, alff_rois, regions_names):
    """
...
def make_roi_fc_couples_df(sim_fc, fc_combinations, regions_names):
    """
...
def make_roi_fc_mean_df(sim_fc, fc_regions, regions_names):
    """
    ...
    for region in fc_regions:
        ...
        fc = (np.mean(sim_fc[idx1], axis=0)) / 2
        name = f'{region}_FC'
        fc_dic.update({name:fc})
